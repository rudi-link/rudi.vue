  ...
  <div v-if="authStore.user" class="flex flex-col w-full h-full">
    ...
    <div class="flex sm:flex-row flex-col-reverse w-full h-full bg-white">
      ...
      >
        ...
        <div
          v-if="withProfile"
          class="hidden md:flex flex-col gap-2 min-w-[17rem] h-max"
        >
          <div
            class="flex items-center justify-center font-bold text-lg font-mono h-11 text-center rounded bg-indigo-50 text-neutral-700 hover:animate-pulse transition-all"
          >
            {{ authStore?.user?.name }}
          </div>
          <div class="flex flex-col gap-5 items-center py-6 px-4 w-full h-max min-h-10 bg-indigo-50 rounded-lg">
